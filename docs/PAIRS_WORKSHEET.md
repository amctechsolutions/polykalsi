...
| **Definitional diff notes** | *(operator fill-in — flagged: Kalshi's close (17:59 UTC) is ~2 hours after a typical 2pm ET FOMC statement; Polymarket's `endDate` (00:00 UTC same day) reads as a nominal "event day" marker, not the real cutoff — confirm which timestamp each venue actually enforces before trusting book-age math on this pair)* | |
...
| **Recommended?** | *(operator fill-in — my read: likely N as a direct pair given the structural mismatch, but that's exactly the judgment call that's yours)* | |
...
| **Definitional diff notes** | *(two real things, not boilerplate: (1) Kalshi checks status at a SINGLE INSTANT — 10am ET Oct 1 — while Polymarket checks a WINDOW through 11:59pm ET the same day, ~14 hours longer; a shutdown beginning at, say, 6pm ET Oct 1 would resolve Polymarket YES but Kalshi would already have settled NO hours earlier. (2) Liquidity is asymmetric — Kalshi ~$355k vs Polymarket ~$7.5k total ($116 24h) — a thin Polymarket quote next to a deep, tight Kalshi one could produce phantom edges on `executable_top_size` alone; worth deciding whether this pair needs a minimum-size floor)* | |
| **Recommended?** | *(operator fill-in)* | |
